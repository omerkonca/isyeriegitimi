...
//void update_data() {
//    wwa += ((float)rand() / (float)RAND_MAX) * 0.1f;
//    rwa += ((float)rand() / (float)RAND_MAX) * 0.1f;
//}
//
//int main() {
//    srand(0);
...
void UpdateData2(float d) {
    for (int i = 0; i < 50; i++) {
        ...
        if (d < 0.25f) {
            ...
            xs2[i] += (d - 0.25f) * 4.0f * 0.2f;
        }
        else if (d < 0.75f) {
            ys2[i] += (d - 0.5f) * 4.0f * 0.2f;
        }
        else {
            xs2[i] -= (d - 0.75f) * 4.0f * 0.2f;
        }
    }
}
// Main code
int main(int, char**)
{
    ...
    // IP adresi saklanacak değişken
    char ipAddress[16] = "1.1.1.1";
    bool page1 = true;
...
#endif
    {
        ...
        //    // Örnek olarak, saklanan IP adresini konsola yazdırma:
        //    std::cout << "Girilen IP Adresi: " << ipAddress << std::endl;

        //    // Yeni sayfayı açın ve IP adresi bilgisini aktarma
        ...
        //    ImGui::Text("Lütfen geçerli bir IP adresi girin.");
        ...
        if (!page1) {
            ...
            ImGui::Text("Entered IP Address: %s", ipAddress); // Girilen IP adresini göstermek için metin kutusu oluşturma
            ImGui::SetNextWindowSize(ImVec2(920, 520));
           ...
           // static RollingBuffer rdata1, rdata2;
            ...
            if (ImGui::CollapsingHeader("AGV"))
            {
                if (ImGui::BeginTable("split", 3))
                {
                    ...
                    if (position == true)//position checkbox ına tıklanınca
                    {
                        ImGui::Begin("Position Chart");
                        // Yeni sayfa içeriği ve IP adresi bilgisi
                        --------------------------------------------------
                    ...
                    if (angle == true) //angle checkbox ına tıklanınca
                    {
                    ...
                    if (speed == true)//speed checkbox ına tıklanınca
                    {
                        ...
                        /* float wwa = 0.0f;
                         float rwa = 0.0f;
                             ...
                             if (ImPlot::BeginPlot("Scatter Plot")) {
                                 ...
                                 while (!ImGui::IsWindowCollapsed()) {
                                     update_data();
                                     static float xs2[1], ys2[1];
                                     xs2[0] = wwa;
                                     ys2[0] = rwa;
                                     ImPlot::SetNextMarkerStyle(ImPlotMarker_Up, 6, ImPlot::GetColormapColor(1), IMPLOT_AUTO, ImPlot::GetColormapColor(1));
                                     ImPlot::PlotScatter("Data 2", xs2, ys2, 1);
                    ...
                    if (wheel == true)//wheel checkbox ına tıklanınca
                    {
                        ImGui::Begin("Wheel Chart");
                        // Kodlarınızı burada yerleştirin
                        ImGui::End();
           ...
           //--------------------------------------------------
            ...
            //// Yeni sayfa butonları
            //if (ImGui::Button("Ok")) {
            //    ImGui::CloseCurrentPopup(); // Pencereyi kapatın
            //    
            //}
